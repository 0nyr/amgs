    ...
    private void checkBlankSpace() {
        ...
        if(Constants.isCameraLimitedToGameArea) {
            if(xOffset < 0) {
                ...
                handler.getWorld().getWorldWidthInTile()*Constants.TILE_WIDTH 
                - handler.getDisplayWidth()) 
                    {
                xOffset = handler.getWorld().getWorldWidthInTile()*Constants.TILE_WIDTH 
                    - handler.getDisplayWidth();
            }
    
            if(yOffset < 0) {
                yOffset = 0;
            } else if(yOffset > handler.getWorld().getWorldHeightInTile()*Constants.TILE_HEIGHT
                - handler.getDisplayHeight())
                    {
                yOffset = handler.getWorld().getWorldHeightInTile()*Constants.TILE_HEIGHT
                    - handler.getDisplayHeight();
            }
        }
    }
